...
class CollectionAdmin(ImportExportModelAdmin):
    ...
    #             get_url = f"{url}?name={obj.name}&business_code={obj.business_code}"
    #             get_response = requests.get(get_url, headers=headers)
    #             get_response.raise_for_status()
    #             get_response_json = get_response.json()
    #             if get_response_json.get("result") == []:
    #                 response = requests.post(url, data=form_data, headers={'Authorization': f"Bearer {token}"})
    #             else:
    #                 existing_id = get_response_json['result'][0]['id']
    #                 put_url = f"{url}{existing_id}/"
    #                 response = requests.put(put_url, data=form_data, headers=headers)
    #
